...
select 'power:' || power_mode || ',screen:' || screen_mode || ',thermal:' || thermal_mode as name
	...
	, case
		when sum(total_eg) / 1000000.0 < 2500.0
...
from (
	...
	from (
		...
		from (
			...
			from agg_batteryAgent_daily, json_each(agg_batteryAgent_daily.energy)
		)
...
from (
	...
	from (
		...
		from (
			...
			from (
				select value as one_mode
				from agg_batteryAgent_daily, json_each(agg_batteryAgent_daily.by_mode)
			)
		) T3, json_each(T3.total_eg)
	) T4;


--- per app power mode check
select name || '_power_mode:' || power_mode as APP
	, sum(total_eg) / 1000000.0 as mAh, case when sum(total_eg) / 1000000.0 < 1000.0 and sum(total_eg) / 1000000.0 >= 0 then 'pass' else 'fail' end as result
from (
	select power_mode, screen_mode, thermal_mode, value as total_eg, name
	from (
		select json_extract(one_mode, '$.total_eg') as total_eg
			, json_extract(one_mode, '$.power_mode') as power_mode
			, json_extract(one_mode, '$.screen_mode') as screen_mode
